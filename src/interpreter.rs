...
pub enum Flow { None, Break }
pub struct Symbol { pub value: i32, pub is_mutable: bool }
pub struct Environment { pub memory: HashMap<String, Symbol> }
    ...
    pub fn execute(&mut self, stmts: Vec<Stmt>) -> Result<Flow, String> {
        for stmt in stmts {
            match stmt {
                ...
                Stmt::Declaration { name, is_mutable, value } => {
                    let v = self.eval(&value)?;
                    self.memory.insert(name, Symbol { value: v, is_mutable });
                }
                Stmt::Assignment { name, value } => {
                    let v = self.eval(&value)?;
                    let sym = self.memory.get_mut(&name).ok_or("Inexistente")?;
                    if !sym.is_mutable { return Err("Imutavel".into()); }
                    sym.value = v;
                }
                Stmt::Print(e) => println!("{}", self.eval(&e)?),
                Stmt::If { condition, then_body, else_body } => {
                    if self.eval(&condition)? != 0 {
                        if let Flow::Break = self.execute(then_body)? { return Ok(Flow::Break); }
                    } else if let Some(e_body) = else_body {
                        if let Flow::Break = self.execute(e_body)? { return Ok(Flow::Break); }
                    }
                }
                Stmt::Repeat { body } => {
                    loop {
                        if let Flow::Break = self.execute(body.clone())? { break; }
                    }
                }
            }
        }
